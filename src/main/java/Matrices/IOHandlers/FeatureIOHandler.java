...
/**
 * A class with methods that handle exporting, saving, or importing a FeatureDSMData matrix object
 * Extended to support Thebeau matlab files (.m). Implements saving to CSV and Excel files
 *
 * @author Aiden Carney
 */
public class FeatureIOHandler extends AbstractIOHandler implements IThebeauExport {
     ...
     */
    public FeatureIOHandler(File file) {
        super(file);
    }
     ...
     */
    public FeatureIOHandler(File file, FeatureDSMData matrix) {
        super(file);
        this.matrix = matrix;
    }
     ...
     */
    @Override
    public FeatureDSMData readFile() {
        try {
            ...
            FeatureDSMData matrix = new FeatureDSMData(matrixGroupings.values());
    ...
    /**
     * Reads a matlab file in Thebeau's format and parses it as a FeatureDSMData object. Returns the FeatureDSMData object,
     * but does not automatically add it to be handled.
     *
     * @param file  the file location to read from
     * @return      FeatureDSMData object of the parsed in matrix
     */
    public FeatureDSMData importThebeauMatlabFile(File file) {
        FeatureDSMData matrix = new FeatureDSMData();
    ...
    /**
     * Reads a csv file as an adjacency matrix and parses it into a FeatureDSMData object and returns it. Does not
     ...
     * @return      FeatureDSMData object of the parsed in matrix
     */
    public FeatureDSMData importAdjacencyMatrix(File file) {
        FeatureDSMData matrix = new FeatureDSMData();
    ...
    public int saveMatrixToFile(File file) {
        try {
            ...
            infoElement.addContent(new Element("type").setText("feature"));
            ...
            for(Grouping group: ((FeatureDSMData) matrix).getGroupings()) {
    ...
    public int exportMatrixToAdjacencyMatrix(File file) {
        try {
            StringBuilder contents = new StringBuilder("feature\n");
