...
public class FeatureView extends AbstractMatrixView implements ISymmetricHighlight {

    private boolean symmetryValidation = false;

    FeatureDSMData matrix;

    /**
     * Returns a MatrixGuiHandler object for a given matrix
     *
     * @param matrix   the FeatureDSMData object to display
     * @param fontSize the default font size to display the matrix with
     */
    public FeatureView(FeatureDSMData matrix, double fontSize) {
     ...
     * @return          this
     */
    public FeatureView withFontSize(double fontSize) {
     ...
     * @return      this
     */
    public FeatureView withMode(MatrixViewMode mode) {
    ...
    /**
     * Copy constructor for FeatureView class. Performs a deep copy on the matrix data. Everything else will be
     * generated when the refreshView method is called
     *
     * @return  the copy of the current FeatureView
     */
    @Override
    public FeatureView createCopy() {
        FeatureView copy = new FeatureView(matrix.createCopy(), fontSize.doubleValue());
    ...
    public void refreshCellHighlight(Cell cell) {
        if (cell.getHighlightBG("error") != null) {
            ...
            if (rowUid == null && colUid != null) {  // highlight with column color
                ...
                // have same group in feature matrix
                cell.setCellHighlight(matrix.getRowItem(rowUid).getGroup1().getColor());
                cell.setCellTextColor(matrix.getRowItem(rowUid).getGroup1().getFontColor());
            } else {
    ...
    protected void refreshEditView() {
        ...
        for(int r=0; r<rows; r++) {
            ...
            for(int c=0; c<columns; c++) {
                ...
                rowData.add(cell);
                if(label != null) {
                    var cell_fontSize = new SimpleDoubleProperty(label.textProperty().length().greaterThan(3).get()?fontSize.subtract(3).get():fontSize.get());
                    Cell cellObject = new Cell(new Pair<>(r, c), cell, label, cell_fontSize);
                }
        ...
        // only run this section if symmetryValidation is enabled because its cells default to not being highlighted,
    ...
    protected void refreshStaticView() {
        ...
        for(int r=0; r<rows; r++) {
            for(int c=0; c<columns; c++) {
                ...
                var cell_fontSize = new SimpleDoubleProperty(label.textProperty().length().greaterThan(3).get()?fontSize.subtract(3).get():fontSize.get());
                Cell cellData = new Cell(new Pair<>(r, c), cell, label, cell_fontSize);
